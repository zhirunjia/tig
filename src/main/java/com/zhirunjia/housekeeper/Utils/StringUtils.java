...
/**
 * Created by Xu wenQiang on 2014/6/25.
 */
public class StringUtils {
    ...
    public static byte[] hexStr2Bytes(String hexString) {
        ...
        int length = hexString.length() / 2 + hexString.length() % 2;
        char[] hexChars = hexString.toCharArray();
        byte[] hexByte = new byte[length];
        for (int i = 0; i < length; i++) {
            int pos = i * 2;
            hexByte[i] = pos + 1 < hexChars.length ? (byte) (charToByte(hexChars[pos]) << 4 | charToByte(hexChars[pos + 1])) : charToByte(hexChars[pos]);
        }
        return hexByte;
    }
